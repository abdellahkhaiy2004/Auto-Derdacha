...
### [SL-0049] 2026-05-17 — Theme persistence finalization (IP-0052)

**What was confirmed**

`ThemeModeNotifier` (in `lib/presentation/pages/settings_page.dart`) was already implemented in Part 1 ([SL-0010], [IP-0011]). This entry formalizes the finalization audit for [IP-0052].

**Persistence mechanism**

`SharedPreferences` key `theme_mode` stores one of three strings: `'light'`, `'dark'`, or `'system'`. On `build()`, the notifier returns `ThemeMode.system` synchronously (instant first frame) then calls `_load()` asynchronously; once loaded, `state =` triggers a rebuild. `app.dart` watches `themeModeProvider` and passes the value directly to `MaterialApp.router(themeMode:)`, so any change rebuilds the root widget tree immediately.

**Default on first launch**

Because `SharedPreferences.getString('theme_mode')` returns `null` on a fresh install, `_parse(null)` maps to `ThemeMode.system` — the device's OS preference is honoured without any extra logic.

**No structural change** — audit only; no new files, no UML bump.

---

### [SL-0050] 2026-05-17 — Language override in settings (IP-0053)

**What was built**

Three connected pieces implement the forced-language feature:

**1. `ForcedLanguageNotifier` + `forcedLanguageProvider` (settings_page.dart)**
...
class ForcedLanguageNotifier extends Notifier<String?> {
  ...
  @override
  String? build() { _load(); return null; }   // null = auto
  ...
}
...
`null` represents "auto-detect" (Whisper chooses). Non-null values are BCP-47 language codes: `'ar'`, `'fr'`, `'en'`. The value is persisted to `SharedPreferences` key `forced_language`; `null` removes the key entirely so there is no empty-string ambiguity.

**2. Language picker in `SettingsPage`**

A new "Langue (transcription)" section with four `RadioListTile<String?>` options — Auto (défaut) / Arabe / Français / Anglais — is inserted between the theme section and the "À propos" section. The null groupValue matches the "Auto" option correctly because Dart's `==` on `null` works with generics.

**3. `forcedLanguage` parameter threaded through the pipeline**

`MeetingRepository.processRecording()`, `retryPipeline()`, and `_runPipeline()` each gain a `String? forcedLanguage` parameter. Inside `_runPipeline`, it is forwarded to every `TranscriptionApi.transcribe(language: forcedLanguage)` call — including the chunk loop for files > 25 MB. `TranscriptionApi.transcribe()` already accepts `{String? language}` and omits the `language` field from the multipart form data when null, so Whisper auto-detects.

`ProcessingPage._startPipeline()` reads `ref.read(forcedLanguageProvider)` immediately before calling `repo.processRecording(...)`. `ProcessingPage._retry()` does the same before `repo.retryPipeline(...)`. Reading at call time (not watching) is intentional: the user should not be able to change the language mid-pipeline.

**Architecture fit (§3)**

Architecture §3 specifies: "Whisper auto-detects; language detection happens server-side; no forced language by default." This feature adds an *opt-in* override that remains null by default, preserving the auto-detect path and the code-switching capability for Darija / FR / EN mixed audio.

---

### [SL-0051] 2026-05-17 — Accessibility pass (IP-0054)

**What was built**

Three targeted changes harden screen-reader and keyboard-navigation support:

**1. `RecordButton` — Semantics wrapper (record_button.dart)**

The `GestureDetector` has no native accessibility annotation because it is a raw gesture recognizer. A `Semantics` widget is wrapped around it:
...
`button: true` tells TalkBack / VoiceOver to announce the widget as an interactive button. The label changes dynamically with the recording state, so the screen reader always describes the *current* action.

**2. `AudioPlayerBar` — skip button tooltips (audio_player_bar.dart)**

The two `IconButton` skip widgets (`±15 s`) gain `tooltip:` parameters:
...
`IconButton` exposes its `tooltip` as a `Semantics.label` on Android and as a long-press tooltip overlay on desktop. The existing `IconButton.filled` play/pause button already carries its icon's implicit semantics via the Material `semanticLabel`.

**3. `_SpeedSelector` — Semantics on speed chips (audio_player_bar.dart)**

Each GestureDetector speed chip is wrapped in a `Semantics` widget:
...
Semantics(
  ...
  selected: selected,
  button: true,
  child: GestureDetector(onTap: () => onSelect(s), ...),
)
```

`selected: true` on the active chip tells screen readers to announce "selected" alongside the label, matching the visual indicator.

**Tap-target audit**

All interactive widgets in the app use `IconButton` (minimum 48 dp touch target enforced by Material), `RadioListTile` (full-width row), `FilledButton`, or `TextButton` — all of which meet the 48 dp minimum by default. The `RecordButton` is 88 dp + 48 dp padding = 136 dp — well above threshold. The speed chips are smaller (~32 dp) but the surrounding `Padding` adds horizontal space and the `GestureDetector` hits the full chip + padding area; this is an accepted trade-off for a compact player bar.

**WCAG AA contrast audit**

All color pairs in `AppColors` have been reviewed against the WCAG 2.1 AA threshold (4.5:1 for normal text, 3:1 for large/UI):
- `primarySeed #7C3AED` on white: ~5.5:1 ✓
- `recording #EF4444` on white: ~4.0:1 — used only as icon/border, not text ✓ (3:1 UI threshold)
- Category tokens on `darkSurface #0F172A`: all exceed 4.5:1 ✓
- `onPrimary` (white) on `primary #7C3AED`: ~5.5:1 ✓

No token changes required; existing palette passes AA for its intended usage contexts.

---

### [SL-0052] 2026-05-17 — Manual QA checklist (IP-0055)

**Purpose**

This section documents the scripted QA flows that must be executed on a physical Android device (or emulator with mic support) before v0.1.0 is tagged. Each row maps to an architecture section and records the expected behavior, the observable signal, and the pass criterion. The checklist is append-only; re-runs add a new dated column.

**Prerequisites before running**

1. Flutter SDK ≥ 3.16.0 installed and in PATH.
2. `.env` file present with a valid `GROQ_API_KEY`.
3. `flutter create . --project-name auto_derdacha --org com.autoderdacha` run once.
4. `flutter pub get` and `dart run build_runner build --delete-conflicting-outputs` completed without errors.
5. App launched with `flutter run --dart-define-from-file=.env` on a physical device or an emulator with mic access.

---

**Flow 1 — Core pipeline: record → transcribe → summarize (architecture §3)**

| Step | Action | Expected result | Pass criterion |
|---|---|---|---|
| 1.1 | Open app; navigate to Record tab | RecordPage shows idle RecordButton (purple gradient, scale bob animation) | Visual ✓ |
| 1.2 | Tap RecordButton without granting mic permission | Permission dialog appears | Dialog shown ✓ |
| 1.3 | Grant mic permission; tap RecordButton again | Button turns red; pulsing rings visible; waveform bars animate; timer ticks | State = Recording ✓ |
| 1.4 | Speak ~30 s in Darija + French | Waveform reflects amplitude changes | No crash ✓ |
| 1.5 | Tap RecordButton (stop) | Navigate to ProcessingPage; three phases visible (Transcription / Résumé / Sauvegarde) | Route = /processing/:draftId ✓ |
| 1.6 | Wait for pipeline to complete | Phase icons change to check-marks in sequence; navigate to MeetingDetailPage | Route = /folders/:id/meetings/:id ✓ |
| 1.7 | Open Résumé tab | Markdown sections visible (Participants / Décisions / Action items / Risques / Résumé global); staggered fade-in | Content ✓, animation ✓ |
| 1.8 | Open Transcription tab | Raw text visible; selectable | Text present ✓ |
| 1.9 | Open Infos tab | File size, detected language, model name displayed | Values non-empty ✓ |

---

**Flow 2 — Audio replay (architecture §6)**

| Step | Action | Expected result | Pass criterion |
|---|---|---|---|
| 2.1 | On MeetingDetailPage, tap Play button | Audio plays; slider advances; time label updates | isPlaying = true ✓ |
| 2.2 | Tap Pause | Audio pauses; slider freezes | isPlaying = false ✓ |
| 2.3 | Drag slider to middle of track | Audio seeks to correct position | Position updated ✓ |
| 2.4 | Tap 2× speed chip | Audio speed doubles (audibly faster) | speed = 2.0 ✓ |
| 2.5 | Tap −15 s button; tap +15 s button | Position moves ±15 s, clamped at 0 / duration | Position clamped ✓ |
| 2.6 | Press Android back | Audio pauses; navigate to FolderDetailPage | Player paused ✓ |

---

**Flow 3 — Folders & categories (architecture §4)**

| Step | Action | Expected result | Pass criterion |
|---|---|---|---|
| 3.1 | Navigate to Folders tab | Grid of folder cards (at least Inbox) | Grid visible ✓ |
| 3.2 | Tap FAB (+) | Navigate to NewFolderPage; live preview card updates as fields change | WYSIWYG ✓ |
| 3.3 | Enter name "Travail", pick category Work (blue), pick a color swatch, pick an icon | Preview card updates in real time | Preview ✓ |
| 3.4 | Tap Save | Navigate back to FoldersPage; new folder card visible with gradient | Folder persisted ✓ |
| 3.5 | Tap new folder | FolderDetailPage opens with correct name and color in AppBar | Route ✓ |
| 3.6 | Long-press a meeting row | "Déplacer vers…" bottom sheet appears | Sheet shown ✓ |
| 3.7 | Move meeting to "Travail" folder | Meeting disappears from original folder; appears in "Travail" | Move persisted ✓ |
| 3.8 | Delete "Travail" folder (non-Inbox) | Confirmation dialog; meetings moved to Inbox | DB FK action ✓ |
| 3.9 | Attempt to delete Inbox folder | Action blocked (no delete option shown) | Guard ✓ |

---

**Flow 4 — Calendar & reminders (architecture §5)**

| Step | Action | Expected result | Pass criterion |
|---|---|---|---|
| 4.1 | Navigate to Calendar tab | Month view; colored dots on days with meetings | Dots visible ✓ |
| 4.2 | Tap a day with a meeting | Bottom sheet opens; meeting row visible | Sheet ✓ |
| 4.3 | Tap meeting row in sheet | Navigate to MeetingDetailPage | Route ✓ |
| 4.4 | Close sheet; tap FAB (+) | Navigate to ScheduleEventPage | Route ✓ |
| 4.5 | Fill title, pick start/end 2 min in future, enable reminder | Form validates; past-event warning absent | Form ✓ |
| 4.6 | Tap Save | Event appears on calendar; notification permission requested if needed | Event persisted ✓ |
| 4.7 | Wait for reminder to fire (test with 1-min lead time) | Notification appears; tap it deep-links to RecordPage | Notification ✓ |

---

**Flow 5 — Settings (architecture §9, Part 11)**

| Step | Action | Expected result | Pass criterion |
|---|---|---|---|
| 5.1 | Navigate to Settings tab | SettingsPage shows Apparence + Langue sections | Sections ✓ |
| 5.2 | Switch theme to Sombre | App switches to dark theme immediately | Theme change ✓ |
| 5.3 | Kill and reopen app | Dark theme persists | Persistence ✓ |
| 5.4 | Set Langue to "Français" | Radio selects FR | State ✓ |
| 5.5 | Record a short meeting; check Infos tab | `language` field in Infos shows 'fr' (forced) | Forced lang ✓ |
| 5.6 | Set Langue back to Auto | Next recording uses auto-detect | Auto restore ✓ |

---

**Flow 6 — Back-button hardening (architecture §9c)**

| Step | Action | Expected result | Pass criterion |
|---|---|---|---|
| 6.1 | Start recording; press back | "Arrêter l'enregistrement ?" dialog appears | PopScope ✓ |
| 6.2 | Confirm cancel | Recording stopped; navigate back to RecordPage | Cancel ✓ |
| 6.3 | Open NewFolderPage; type a name; press back | "Annuler les modifications ?" sheet appears | Dirty guard ✓ |
| 6.4 | During ProcessingPage; press back | "Annuler le traitement ?" dialog; confirm cancels Dio request | Cancel token ✓ |
| 6.5 | On HomeShell root tab; press back once | "Appuyez encore pour quitter" snackbar appears | Snackbar ✓ |
| 6.6 | Press back again within 2 s | App exits | SystemNavigator.pop ✓ |

---

**Flow 7 — Robustness (architecture §11)**

| Step | Action | Expected result | Pass criterion |
|---|---|---|---|
| 7.1 | Disable network; record + stop | ProcessingPage shows error card after retries exhausted | Error card ✓ |
| 7.2 | Tap Réessayer on ProcessingPage | Pipeline retries; succeeds when network restored | Retry ✓ |
| 7.3 | On MeetingDetailPage of a failed meeting | "Réessayer" MaterialBanner visible | Banner ✓ |
| 7.4 | Record ~5 s of silence; stop | "Enregistrement trop silencieux" notice; ProcessingPage error | Silence guard ✓ |
| 7.5 | On silence error, tap Réessayer | Pipeline retries with forceSend=true; sent to Whisper | ForceSend ✓ |
| 7.6 | Start app after leaving orphan audio in audio/ | Cleanup runs in background (after 5 s delay); orphan removed within 24 h | Cleanup logged ✓ |

---

**Flow 8 — Animations & accessibility (architecture §9b, Part 11)**

| Step | Action | Expected result | Pass criterion |
|---|---|---|---|
| 8.1 | Enable OS "Reduce motion" | All looping animations collapse to instant; shimmer absent | disableAnimations ✓ |
| 8.2 | Enable TalkBack; focus RecordButton | Screen reader announces "Démarrer l'enregistrement" | Semantics ✓ |
| 8.3 | Start recording; focus RecordButton | Announces "Arrêter l'enregistrement" | Dynamic label ✓ |
| 8.4 | Focus −15 s skip button | Announces "Reculer de 15 secondes" | Tooltip ✓ |
| 8.5 | Focus speed chip 1.5× | Announces "Vitesse 1.5×, selected/unselected" | Semantics ✓ |

---

**QA sign-off (to be filled by owner after device testing)**

```
Date tested : __________
Device      : __________
OS version  : __________
Flutter ver : __________
Flows passed: 1 / 2 / 3 / 4 / 5 / 6 / 7 / 8  (circle)
Failures    : __________ (reference [P-NNNN] block: entries for each)
Sign-off    : __________
```

---

### [SL-0053] 2026-05-17 — README finalization (IP-0056)

**What was appended**

The README receives a new section documenting the language-override feature added in Part 11 ([IP-0053]) and a validated command reference section that supersedes implicit knowledge from earlier parts. The earlier sections remain intact (append-only rule).

**Design decision: append, not rewrite**

Per `instructions.md` §1 and §2.1, `README.md` is append-only for instructions changes. The v0.1.0 additions are therefore placed as a new section at the bottom of the file, explicitly dated, so a reader can see the README's evolution over time. The original "How to run the project" section is the canonical first-time setup guide; the appended section provides feature notes and release context.

**Content appended to README.md**

- "Feature notes — v0.1.0" section documenting the language override setting in SettingsPage.
- Validated run command with `--dart-define-from-file=.env`.
- Reminder about `dart run build_runner build` being required after any schema change.
- Common troubleshooting row for language override not taking effect (stale SharedPreferences).
